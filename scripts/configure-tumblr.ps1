...
$apiKey = (Read-SecretText 'Tumblr API key (OAuth consumer key) from https://www.tumblr.com/oauth/apps').Trim()
if ($apiKey.Length -lt 10) {
    throw 'The Tumblr API key appears incomplete.'
}

$blogs = (Read-Host 'Comma-separated Tumblr blog identifiers to follow (e.g. waroftrights.tumblr.com)').Trim()
if (-not $blogs) {
    throw 'At least one Tumblr blog identifier is required.'
}
...
try {
    [IO.File]::WriteAllText($tempFile, $apiKey, [Text.UTF8Encoding]::new($false))
